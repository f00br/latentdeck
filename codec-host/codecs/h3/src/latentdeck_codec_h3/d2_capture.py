"""Bounded Snapshot and Live Capture state for the H3 LD-D2 worker."""
...
MAX_CAPTURE_CONTROL_EVENTS = 32


class D2CaptureError(RuntimeError):
...
class D2CaptureSession:
    """Own one disk spool from restart barrier through bounded receipt."""
    ...
    def __init__(
        ...
        *,
        ...
        if mode not in CAPTURE_MODES:
            raise D2CaptureError("capture.mode_invalid", "capture mode is not supported")
        self.mode = mode
        self._sources = {"A": source_a, "B": source_b}
        routing = controls.get("routing")
        if routing not in self._sources:
            raise D2CaptureError("capture.controls_invalid", "structural carrier is invalid")
        self.structural_carrier = str(routing)
        ...
        try:
            ...
            raise D2CaptureError("capture.start_failed", "capture spool could not start") from error
    ...
    def activate(self, reset_result: Mapping[str, object]) -> None:
        if self._state != "awaiting_reset":
            raise D2CaptureError("capture.invalid_state", "capture is not awaiting its reset")
        generation = reset_result.get("stream_generation")
        if (
            isinstance(generation, bool)
            or not isinstance(generation, int)
            or generation < self._minimum_new_generation
            or reset_result.get("playhead_a") != 0
            or reset_result.get("playhead_b") != 0
        ):
    ...
    def after_decode(self, step: D2ProcessedSlot) -> None:
        """Finalize only after decode confirmed the captured pre-decode slot."""
    ...
    def request_stop(self) -> None:
        if self.mode != "live_capture":
            raise D2CaptureError("capture.mode_invalid", "Snapshot capture stops automatically")
        if self._state == "finished" or self._state == "stop_armed":
            return
        if self._state == "awaiting_reset":
            self.abort("stopped_before_start")
            return
        if self._state != "capturing":
            raise D2CaptureError("capture.invalid_state", "Live Capture is not running")
    ...
    def ensure_event_capacity(self) -> None:
        if self.accepts_live_events and len(self._control_events) >= MAX_CAPTURE_CONTROL_EVENTS:
            raise D2CaptureError(
                "capture.event_limit", "Live Capture control-event history is full"
            )

    def record_control_state(self, controls: Mapping[str, object], seed: int) -> None:
        ...
        if controls.get("routing") != self.structural_carrier:
            self._carrier_mapping_changed = True
        _bounded_json(event, "capture control event")
    ...
    def _finish(self) -> None:
        ...
        receipt: dict[str, object] = {
            ...
            "parents": [
                ...
                for slot, source in (("A", self._sources["A"]), ("B", self._sources["B"]))
            ],
        }
...
def _capture_id(value: str) -> str:
    try:
        ...
        raise D2CaptureError("capture.id_invalid", "capture_id is not a UUID") from error
    if parsed.int == 0 or str(parsed) != value:
        raise D2CaptureError("capture.id_invalid", "capture_id is not canonical")
    return value
...
def _bounded_json(value: Mapping[str, object], label: str) -> str:
    try:
        ...
        raise D2CaptureError("capture.provenance_invalid", f"{label} is not JSON-safe") from error
    if len(encoded.encode("utf-8")) > MAX_CAPTURE_RECEIPT_BYTES:
        raise D2CaptureError("capture.receipt_too_large", f"{label} exceeds its byte limit")
    return encoded
