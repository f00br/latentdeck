...
def test_q4_package_is_closed_and_faceplate_exposes_the_exact_contract() -> None:
    ...
    roles = ["carrier", "donor_b", "donor_c", "donor_d"]
    assert signal["slots"] == 4
    assert signal["default_permutation"] == roles
    ...
    assert len(control_by_id) == len(controls) == 15
    ...
    assert role_editors[0]["role_ids"] == roles
    barycentric = [widget for widget in widgets if widget["kind"] == "barycentric3"]
    assert len(barycentric) == 1
    assert barycentric[0]["vertex_role_ids"] == ["donor_b", "donor_c", "donor_d"]
    ...
    for widget in widgets:
        ...
        if widget["kind"] == "barycentric3":
            bindings.extend([widget["x_control_id"], widget["y_control_id"]])
    assert Counter(bindings) == Counter({control_id: 1 for control_id in control_by_id})


def test_q4_integrity_catalog_and_deterministic_ld_are_exact(tmp_path: Path) -> None:
    manifest = strict_json(PACKAGE_ROOT / "deck-pack.json")
    ...
    assert not catalog_paths & {
        "python/latentdeck_operator_q4/descriptor.json",
        "python/latentdeck_operator_q4/descriptor.py",
        "python/latentdeck_operator_q4/descriptor.schema.json",
        "python/latentdeck_operator_q4/stream.py",
        "python/latentdeck_operator_q4/trusted.py",
    }
    ...
    first = tmp_path / "q4-first.ld"
    second = tmp_path / "q4-second.ld"
    ...
    code = textwrap.dedent(
        ...
        loaded = pathlib.Path(latentdeck_operator_q4.__file__).resolve()
        assert loaded.is_relative_to(package_python), loaded
        assert "latentdeck_codec_host" not in sys.modules
        assert not hasattr(latentdeck_operator_q4, "process_slot")
        assert not hasattr(latentdeck_operator_q4, "Q4Context")
        index = torch.arange(96, dtype=torch.float32).reshape(1, 8, 1, 3, 4)
        sources = tuple(
            torch.sin(index * scale).contiguous() for scale in (0.1, 0.2, 0.3, 0.4)
        )
        ...
        result = latentdeck_operator_q4.process_sources(
            sources,
        ...
        for name, module in tuple(sys.modules.items()):
            if name.startswith("latentdeck_operator_q4") and getattr(module, "__file__", None):
...
def test_q4_wheel_build_uses_the_same_authoritative_package_tree(tmp_path: Path) -> None:
    ...
    assert completed.returncode == 0, completed.stderr
    wheel = next(wheel_directory.glob("latentdeck_operator_q4-0.2.0-*.whl"))
    with zipfile.ZipFile(wheel) as archive:
        assert archive.read("latentdeck_operator_q4/operator.py") == source.read_bytes()
        assert "latentdeck_operator_q4/stream.py" not in archive.namelist()
        assert "latentdeck_operator_q4/trusted.py" not in archive.namelist()
    ...
    code = textwrap.dedent(
        ...
        import latentdeck_operator_q4
        assert str(wheel) in latentdeck_operator_q4.__file__
        assert latentdeck_operator_q4.DECK_ID == "org.latentdeck.deck.q4"
        assert "latentdeck_codec_host" not in sys.modules
        """
    )
