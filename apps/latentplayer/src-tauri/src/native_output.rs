//! Raw Tauri window and DX12 presentation surface for decoded Player frames.
//!
//! This boundary deliberately contains no `WebView`, audio, seek state, codec
//! logic, or backend fallback. A later Player actor can own one
//! [`NativeOutput`] and feed it validated RGB Ring frames sequentially.
...
/// Stable Tauri label for the one native decoded-frame output window.
...
/// Fixed decoded-program dimensions used to allocate the native frame texture.
...
impl NativeOutputConfig {
    ...
    #[must_use]
    pub const fn new(frame_width: u32, frame_height: u32) -> Self {
        Self {
            frame_width,
            frame_height,
        }
...
/// Sanitized native-output failure suitable for the later Player actor/API.
///
...
impl NativeOutputError {
    /// Stable machine-readable error code for a future typed Player event.
    #[must_use]
...
impl NativeOutput {
    ...
    ///
    /// The window starts hidden so the caller can show it only after the Player
    /// has a valid decoded frame/status. Call this from an async Tauri setup or
    ...
    pub async fn new(
        ...
        let window = WindowBuilder::new(app, NATIVE_OUTPUT_WINDOW_LABEL)
            .title(NATIVE_OUTPUT_WINDOW_TITLE)
            .inner_size(
        ...
        let mut encoder =
            self.dx12
                ...
                .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                    label: Some("LatentPlayer native output present"),
                });
